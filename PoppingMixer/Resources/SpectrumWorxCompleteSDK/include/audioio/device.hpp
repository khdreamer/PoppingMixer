...
class Device
    ...
    : public Common::StackPImpl<Device, 15 * sizeof( void * ) + 7 * sizeof( int )>
#elif defined( __ENVIRONMENT_IPHONE_OS_VERSION_MIN_REQUIRED__ )
    : public Common::StackPImpl<Device,  3 * sizeof( void * )>
#else
    : public Common::StackPImpl<Device,  3 * sizeof( void * ) + 2 * sizeof( unsigned int )>
#endif // DOXYGEN_ONLY
{
public:
    LE_NOTHROW  Device();
    ...
    LE_NOTHROW void LE_FASTCALL_ABI start() const;
    /// \detail
    /// <B>Effect:</B> Stops the streaming on the device.<BR>
    LE_NOTHROW void LE_FASTCALL_ABI stop () const;
    ...
    ///   latest OS versions (4.1+) and selected devices. Disabling sensors and
    ///   related background services during sensitive low latency playback is
    ///   known to help with 'crackling' (buffer underruns due to unfavourable
    ///   scheduling of the audio processing thread). For use cases that do not
    ///   require the minimum latency possible, using a higher value may help
    ///   with buffer underrun problems and even with battery draining to some
    ///   degree.
...
///
/// \brief Provides synchronized/blocking access to a Device instance
///
/// Useful mostly for single-threaded command line applications.
///
////////////////////////////////////////////////////////////////////////////////
...
class BlockingDevice
    ...
    : public Common::StackPImpl<BlockingDevice, 4 * sizeof( void * )>
#else
    : public Common::StackPImpl<BlockingDevice, 14 * sizeof( void * ) + 10 * sizeof( int )>
#endif // DOXYGEN_ONLY
{
public:
    /// \detail
    /// <B>Effect:</B> Creates the blocking wrapper for the given Device object which must outlive the created BlockingDevice object.
    LE_NOTHROW  BlockingDevice( Device const & );
    ...
    LE_NOTHROW Device const & LE_FASTCALL_ABI device() const;
}; // class BlockingDevice
